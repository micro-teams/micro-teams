...
It does two things: it is baked into the verification-email links (leaving the `http://localhost`
default sends every new user a link that points at their own machine), and it pins the backend's
allowed **CORS origin**. If `PUBLIC_URL` is unset the backend falls back to `cors-origin=*` — writes
still work (the API authenticates with a Bearer token, not a cookie), but pinning it to your domain
is cleaner. Get this wrong and every write 403s with `Invalid CORS request`. After editing `.env`,
`docker compose up -d` recreates the affected services with the new values.
